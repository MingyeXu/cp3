...
class SPD_module(nn.Module):
    ...
    def forward(self, pcd_prev, feat_global, K_prev=None):
        ...
        K_curr = self.mlp_delta_feature(torch.cat([feat_child, H], 1))
...
class SPD_module_multi_ST(nn.Module):
    def __init__(self, dim_feat=512, up_factor=1, i=0, radius=1):
        """Snowflake Point Deconvolution"""
        super(SPD_module_multi_ST, self).__init__()
        self.i = i
        self.up_factor = up_factor
        self.radius = radius
        self.mlp_1 = MLP_CONV(in_channel=3, layer_dims=[64, 128])
        ...
        self.skip_transformer = SkipTransformer(in_channel=128, dim=64,n_knn=12)
        self.skip_transformer2 = SkipTransformer(in_channel=128, dim=64,n_knn=24)

        self.mlp_ps = MLP_CONV(in_channel=256, layer_dims=[64, 32])
        self.ps = nn.ConvTranspose1d(32, 128, up_factor, up_factor, bias=False)   # point-wise splitting

        self.up_sampler = nn.Upsample(scale_factor=up_factor)
        self.mlp_delta_feature = MLP_Res(in_dim=256+128, hidden_dim=128, out_dim=128)
    ...
    def forward(self, pcd_prev, feat_global, K_prev=None):
        ...
        # print('K_prev',K_prev.size())
        # exit(0)
        H1 = self.skip_transformer(pcd_prev, K_prev if K_prev is not None else Q, Q)
        H2 = self.skip_transformer2(pcd_prev, K_prev if K_prev is not None else Q, Q)
        # print('H',H.size())
        ...
        # print('K_curr',K_curr.size())
        # exit(0)
        # print(self.up_factor)
        delta = torch.tanh(self.mlp_delta(torch.relu(K_curr))) / self.radius**self.i  # (B, 3, N_prev * up_factor)
        if self.up_factor!=1:
            pcd_prev = self.up_sampler(pcd_prev)
