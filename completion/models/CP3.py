...
import time
import models.modulate_conv as MCONV
...
class MSAP_SKN_decoder(nn.Module):
    ...
    def forward(self, global_feat, point_input):
        ...
        dense_feat = self.encoder(points) # 256，3072

        if self.up_scale >= 2:
            dense_feat = self.expansion1(dense_feat)

        coarse_features = self.af(self.conv_cup1(dense_feat)) # 64，3072
        coarse_high = self.conv_cup2(coarse_features) # 3，3072

        if coarse_high.size()[2] > self.num_fps:

            idx_fps = furthest_point_sample(coarse_high.transpose(1, 2).contiguous(), self.num_fps)
            coarse_fps = gather_points(coarse_high, idx_fps) # 3, 2048
            coarse_features = gather_points(coarse_features, idx_fps) #64, 2048
        else:
        ...
        return coarse_raw, coarse_high, coarse, fine, up_features
...
class Model(nn.Module):
    ...
    def forward(self, x, gt=None, prefix="train", mean_feature=None, alpha=None):
        ...
        if prefix=="train":
            ...
            # print('feat_x',feat_x.size())       
            o_x = self.posterior_infer2(self.posterior_infer1(feat_x))
            q_mu, q_std = torch.split(o_x, self.size_z, dim=1) #completion
            o_y = self.prior_infer(feat_y)
            p_mu, p_std = torch.split(o_y, self.size_z, dim=1) # reconstruction
        ...
        coarse_raw, coarse_high, coarse, fine, up_features = self.decoder(feat, x)
        ...
        if prefix=="train":
            ...
            return fine, loss4, total_train_loss, up_features,feat
        elif prefix=="val":
            if self.eval_emd:
                emd = calc_emd(fine, gt, eps=0.004, iterations=3000)
            else:
                emd = torch.zeros(coarse_raw.size(0)).cuda()
            cd_p, cd_t, f1 = calc_cd(fine, gt, calc_f1=True)

            return {'out1': coarse_raw, 'out2': fine, 'emd': emd, 'cd_p': cd_p, 'cd_t': cd_t, 'f1': f1, 'up_features':up_features,'global_feat':feat}
        elif prefix=="test":
            return {'result': fine, 'up_features':up_features,'global_feat':feat}
...
class Model_step2(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=1024):
        ...
        self.eval_emd = args.eval_emd
        self.dense_feature_size = 256
        self.expand_feature_size = 256

        self.spd1 = SPD_module_multi_ST(dim_feat=1024, up_factor=1, i=1, radius=1)
        self.spd2 = SPD_module_multi_ST(dim_feat=1024, up_factor=1, i=2, radius=1)
        self.spd3_16384 = SPD_module_multi_ST(dim_feat=1024, up_factor=8, i=3, radius=1)

        self.mlp = MLP_CONV(in_channel=256, layer_dims=[128, 128])

        self.lbs_fc = nn.Sequential(nn.Conv1d(16, 64, 1),
                                    nn.BatchNorm1d(64),
                                    nn.ReLU(inplace=False),
                                    nn.Conv1d(64, 128, 1),
                                    nn.BatchNorm1d(128),
                                    nn.ReLU(inplace=False))
        self.lbs_f1 = nn.Sequential(nn.Conv1d(128, 128, 1),nn.BatchNorm1d(128))
        self.lbs_f2 = nn.Sequential(nn.Conv1d(128, 128, 1),nn.BatchNorm1d(128))
        memory_format = torch.contiguous_format
        self.weight = torch.nn.Parameter(torch.randn([128, 128, 1, 1]).to(memory_format=memory_format))
        self.weight2 = torch.nn.Parameter(torch.randn([128, 128, 1, 1]).to(memory_format=memory_format))
    ...
    def forward(self, lbs, fine_feat , fine, global_feat, gt=None, prefix="train", mean_feature=None, alpha=None):
        
        fine = fine.transpose(1, 2).contiguous()
        batch_size = fine.size()[0] 
        num_input = fine.size()[2]
        fine_input = fine.clone() # B，3，N

  
        lbs = lbs.unsqueeze(-1)
        global_feat = global_feat.unsqueeze(-1)
        fine_feat = self.mlp(fine_feat)
        ...
        pcd1, K_prev = self.spd1(fine_input, global_feat, K_prev)
        pcd2, K_prev = self.spd2(pcd1, global_feat, K_prev)
        pcd3, K_prev = self.spd3_16384(pcd2, global_feat, K_prev)

        pcd1 = pcd1.transpose(1, 2).contiguous()
        pcd2 = pcd2.transpose(1, 2).contiguous()
        out = pcd3.transpose(1, 2).contiguous()

        if prefix=="train":
            if self.train_loss == 'cd':

                loss_cd, _ = calc_cd(out, gt)
                loss_pcd1, _ = calc_cd(pcd1, gt)
                loss_pcd2, _ =  calc_cd(pcd2, gt)              
            else:
                raise NotImplementedError('Only CD is supported')
            total_train_loss = loss_cd.mean() +loss_pcd1.mean() + loss_pcd2.mean()
            return out, loss_cd, total_train_loss
        elif prefix=="val":
            if self.eval_emd:
                emd = calc_emd(out, gt, eps=0.004, iterations=3000)
            else:
                emd = torch.zeros(out.size(0)).cuda()
            cd_p, cd_t, f1 = calc_cd(out, gt, calc_f1=True)

            return {'out': out, 'emd': emd, 'cd_p': cd_p, 'cd_t': cd_t, 'f1': f1}
        elif prefix=="test":
            return {'result': out}
